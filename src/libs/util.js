...
const DCH_TOKEN = 'researchPlatform'

export const setDchToken = token => {
  Cookies.set(DCH_TOKEN, token, { expires: config.cookieExpires || 1 })
}
export const getDchToken = () => {
  const token = Cookies.get(DCH_TOKEN)
  return token
}
  ...
  forEach(list, item => {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      ...
      if (showThisMenuEle(item, access)) {
        // console.log("显示："+item.name)
        res.push(obj)
      } else {
        // console.log("不显示显示："+item.name)
      }
    }
  })
  return res
...
const hasAccess = (access, route) => {
  /**
   * 判断当前用户是否登录，如果没有登录则判断，当前访问的路由是否需要登录
   * 如果不需要登录则返回True，如果已经登录则判断是否有相应的权限
   */
  if (route.meta.needLogin !== undefined && route.meta.needLogin === false) {
    // 不需要登录
    return true
  } else {
