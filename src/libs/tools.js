 ...
 * @param id String
 * @param arr Array
 * @description 根据传入数组组成对象
 */
export const arraytoObj = function (arr, obj, id) {
  if (id === undefined) {
    let getId = ''
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].hasOwnProperty('kgType')) {
        if (arr[i].kgType === 0) {
          getId = arr[i].id
          break
        }
...
export const getFormartDate = (tmpDate, type) => {
  if (tmpDate) {
    ...
    let computedDate = ''
    if (type === 'all') {
      computedDate = year + '-' + month + '-' + date + ' ' + hours + ':' + minutes + ':' + second
    } else if (type === 'year') {
      computedDate = year + '-' + month + '-' + date
    } else {
      computedDate = year + '-' + month + '-' + date + ' ' + hours + ':' + minutes
    }
    return computedDate
  }
}
...
/**
 * @param arr Array
 * @description 数组对象去除重复(根据id去重)
 */
export const uniqueArrObject = function (arr) {
  let obj = {}
  arr = arr.reduce((cur, next) => { // 数组去重
